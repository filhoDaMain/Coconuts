...
{   
    ...
    {
        return new GNUWindow(props);
    }
    
    GNUWindow::GNUWindow(const WindowProperties& props)
    {
        Init(props);
    }
    ...
    {
        ShutDown();
        LOG_TRACE("GNUWindow objects freed");
    }
    
    /* Private - Shall only be called from GNUWindow() constructor */
    void GNUWindow::Init(const WindowProperties& props)
    {
        int rc;
        ...
        /* Some necessary Hints for platform GNU before window creation */
        OpenGLGraphicsContext::PreInitHints(TargetPlatform::Platform_GNU);
    ...
    bool GNUWindow::InitWindowManagerCallbacks(const char* library)
    {
        if (strcmp(library, "GLFW") == 0)
        {
            ...
            glfwSetWindowCloseCallback(p_glfwWindow, [](GLFWwindow* window)
            {
                 ...
                 */
                GNUWindowData &thisWinData = *((GNUWindowData*)glfwGetWindowUserPointer(window));
    ...
    /* Private - Shall only be called from ~GNUWindow() deconstructor */
    void GNUWindow::ShutDown()
    {
        glfwDestroyWindow(p_glfwWindow);
    }
    
    void GNUWindow::OnUpdate()
    {
    ...
    void GNUWindow::SetVSync(bool enable)
    {
        if (enable)
        {
            glfwSwapInterval(1);
    ...
    bool GNUWindow::IsVsyncEnabled() const
    {
